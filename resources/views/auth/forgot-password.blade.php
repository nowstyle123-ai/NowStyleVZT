<div style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: #000000; display: flex; align-items: center; justify-content: center; padding: 1.5rem; box-sizing: border-box; z-index: 9999; overflow-y: auto;">
    ...
    <div style="background-color: #09090b; border: 1px solid #27272a; border-radius: 1rem; box-shadow: 0 0 50px -10px rgba(220, 38, 38, 0.4); width: 100%; max-width: 420px; padding: 2.5rem; position: relative; z-index: 10; box-sizing: border-box;">
        ...
        <div style="color: #d4d4d8; font-size: 0.85rem; line-height: 1.4; text-align: center; margin-bottom: 1.5rem; font-family: sans-serif;">
            ¿Olvidaste tu contraseña?, Tranquilo, pon tu correo electrónico con el cual te registraste y te enviaremos un mensaje para recuperar tu cuenta.
        </div>

        @if (session('status'))
            <div style="margin-bottom: 1.25rem; text-align: center; color: #34d399; font-size: 0.85rem; font-weight: 600; font-family: sans-serif;">
                {{ session('status') }}
            </div>
        @endif

        <form method="POST" action="{{ route('password.email') }}" style="display: flex; flex-direction: column; gap: 1.25rem; margin: 0; font-family: sans-serif;">
            @csrf
